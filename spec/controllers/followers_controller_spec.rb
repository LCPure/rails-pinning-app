...
RSpec.describe FollowersController do
  ...
  describe "GET index" do
    ...
    it 'populates @followed with all followed users' do
	  get :index
	  expect(assigns(:followed)).to eq(Follower.all)
    end
  ...
  describe "GET new" do
    ...
    it 'assigns @users to equal the users not followed by @user' do
	  get :new
	  expect(assigns(:users)).to be(@user.not_followed)
    end      
  ...
  describe "POST create" do
    ...
    it 'responds with a redirect' do
	  ...
	  expect(response.redirect?).to_be(true)
    end
 
    it 'creates a follower' do
	  post :create, follower: @follower_hash
	  expect(Follower.where("user_id=? AND follower_id=?", @user_id, @follower_user.id).present?).to be(true)
    end
